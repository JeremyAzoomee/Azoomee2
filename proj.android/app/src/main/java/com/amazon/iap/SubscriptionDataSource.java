...
public class SubscriptionDataSource {
    ...
    private final String[] allColumns = { SQLiteHelper.COLUMN_RECEIPT_ID, SQLiteHelper.COLUMN_USER_ID,
            SQLiteHelper.COLUMN_DATE_FROM, SQLiteHelper.COLUMN_DATE_TO, SQLiteHelper.COLUMN_SKU };
    ...
    private SubscriptionRecord cursorToSubscriptionRecord(final Cursor cursor) {
        ...
        subsRecord.setAmazonReceiptId(cursor.getString(cursor.getColumnIndex(SQLiteHelper.COLUMN_RECEIPT_ID)));
        subsRecord.setAmazonUserId(cursor.getString(cursor.getColumnIndex(SQLiteHelper.COLUMN_USER_ID)));
        subsRecord.setFrom(cursor.getLong(cursor.getColumnIndex(SQLiteHelper.COLUMN_DATE_FROM)));
        subsRecord.setTo(cursor.getLong(cursor.getColumnIndex(SQLiteHelper.COLUMN_DATE_TO)));
        subsRecord.setSku(cursor.getString(cursor.getColumnIndex(SQLiteHelper.COLUMN_SKU)));
        return subsRecord;
    }
    ...
    public final List<SubscriptionRecord> getSubscriptionRecords(final String userId) {
        ...
        final String where = SQLiteHelper.COLUMN_USER_ID + " = ?";
        final Cursor cursor = database.query(SQLiteHelper.TABLE_SUBSCRIPTIONS,
                                             allColumns,
                                             where,
                                             new String[] { userId },
                                             null,
    ...
    public void insertOrUpdateSubscriptionRecord(final String receiptId,
        ...
        final String where = SQLiteHelper.COLUMN_RECEIPT_ID + " = ? and "
                             + SQLiteHelper.COLUMN_DATE_TO
                             + " > 0";
        ...
        if (count > 0) {
            ...
            values.put(SQLiteHelper.COLUMN_DATE_FROM, dateFrom);
            values.put(SQLiteHelper.COLUMN_DATE_TO, dateTo);
            values.put(SQLiteHelper.COLUMN_SKU, sku);
            database.insertWithOnConflict(SQLiteHelper.TABLE_SUBSCRIPTIONS,
                                          null,
                                          values,
    ...
    public boolean cancelSubscription(final String receiptId, final long cancelDate) {
        ...
        final ContentValues values = new ContentValues();
        values.put(SQLiteHelper.COLUMN_DATE_TO, cancelDate);
        final int updated = database.update(SQLiteHelper.TABLE_SUBSCRIPTIONS,
                                            values,
                                            where,
                                            new String[] { receiptId });
