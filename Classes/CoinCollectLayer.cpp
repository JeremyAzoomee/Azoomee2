...
void CoinCollectLayer::addPlinth()
{
	...
	_oomee = Sprite::create("res/defaultOomees/oomee_11.png");
	_oomee->setAnchorPoint(Vec2::ANCHOR_MIDDLE_BOTTOM);
	_oomee->setNormalizedPosition(Vec2(0.5,0.8));
	...
	plinth->runAction(Sequence::createWithTwoActions(MoveBy::create(2, Vec2(0,plinth->getContentSize().height * 1.5)),CallFunc::create([this](){this->addCoinCounter();})));
	plinth->runAction(Repeat::create(Sequence::create(EaseBounceInOut::create(MoveBy::create(0.125, Vec2(5,0))),EaseBounceInOut::create(MoveBy::create(0.25, Vec2(-10,0))), EaseBounceInOut::create(MoveBy::create(0.125, Vec2(5,0))), NULL), 4));
	plinth->addChild(_oomee);
	...
	smoke->setPosVar(Vec2(plinth->getContentSize().width / 1.5,50));
...
void CoinCollectLayer::addCoinCounter()
{
	...
	const Vec2& oomeeWorldPos = this->convertToWorldSpace(_oomee->getPosition()) + Vec2(_oomee->getContentSize().width * _oomee->getAnchorPoint().x,_oomee->getContentSize().height * _oomee->getAnchorPoint().y);
