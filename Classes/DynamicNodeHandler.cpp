...
void DynamicNodeHandler::createDynamicNodeById(const std::string& uniqueId)
{
    //local device folder
    const std::string& ctaPath = getCTADirectoryPath();
    const std::vector<std::string>& folders = DirUtil::getFoldersInDirectory(ctaPath);
    
    for(const std::string& folder : folders)
    {
        const std::vector<std::string>& fileNames = DirUtil::getJsonFilesInDirectory(ctaPath + folder);
...
void DynamicNodeHandler::createDynamicNodeByGroupId(const std::string& groupId)
{
    ...
    for(const std::string& folder : folders)
    {
        if(folder == groupId)
        {
            std::vector<std::string> fileNames = DirUtil::getJsonFilesInDirectory(ctaPath + folder);
...
{
    return DirUtil::getCachesPath() + "DCDECache/";
}
...
void DynamicNodeHandler::removeCTAFiles()
{
    ...
    const std::vector<std::string>& CTAFolders = DirUtil::getFoldersInDirectory(baseLocation);
