...
void BackEndCaller::onChildLoginAnswerReceived(std::string responseString)
{
    ...
    HQDataParser::getInstance()->getContent(StringUtils::format("%s%s%s", ConfigStorage::getInstance()->getServerUrl().c_str(), ConfigStorage::getInstance()->getPathForTag("HOME").c_str(), ChildDataProvider::getInstance()->getChildLoginValue("id").c_str()), "HOME");
}
